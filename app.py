...
try:
    ...
    with res_q:
        ...
        if is_spec:
            st.success("✅ 符合電子級規格 (純度 ≥ 99.99% 且 AA ≤ 100 ppm)")
        else:
            st.error("❌ 未達電子級規格 (純度 ≥ 99.99% 且 AA ≤ 100 ppm)")
    ...
    with tabs[0]:
        ...
        st.write("本表於模型訓練範圍內產生 5000 組候選操作條件，經 ANN 模型預測後，先篩選符合電子級 PMA 品質條件者，再依不同決策目標選取代表性操作模式。節能優先模式以總公用工程熱負荷最低為主，產能優先模式以產品莫耳流率最高為主，綜合平衡模式則同時考量產品純度、AA 殘留量、產品流量與總熱負荷。")
