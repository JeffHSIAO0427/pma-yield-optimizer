...
# 範圍警告邏輯
is_out = (
    not (70 <= T <= 110) or not (10 <= Fin <= 200) or not (0.45 <= Raa <= 0.55) or
    not (1 <= C1_R <= 10) or not (1 <= C1_B <= 10) or not (7000 <= C1_P <= 12000) or
    not (1 <= C2_R <= 10) or not (1 <= C2_B <= 10) or not (7000 <= C2_P <= 12000)
)
if is_out:
    st.warning("⚠️ **警告**：此輸入條件已超出本研究 ANN 模型之核心訓練範圍。預測結果可能不具備可靠性，建議僅作為定向參考。")
...
try:
    ...
    with res_q:
        ...
        if is_spec:
            ...
            st.error("❌ 未達電子級規格")
        st.caption("規格門檻：純度 ≥ 99.99% 且 AA ≤ 100ppm")

    with res_p:
        st.markdown("#### **B. 產能表現**")
        st.metric("Product Molar Flow", f"{m_flow:.4f} kmol/h")
        st.metric("PMA Yield", f"{total_yield:.2f} %")
        with st.popover("📈 產率定義"):
            st.latex(r"Yield = \frac{n_{PMA, product}}{\min(n_{AA, feed}, n_{PGME, feed})} \times 100\%")

    with res_e:
        st.markdown("#### **C. 能耗表現**")
        st.metric("Total Energy Duty", f"{total_energy:.2f} kW")
        st.write(f"• **HT-1 Heater:** {h_ene:.2f} kW")
        st.write(f"• **C1 Exchangers:** {ene_dict['C1_Cond']+ene_dict['C1_Reb']:.2f} kW")
        st.write(f"• **C2 Exchangers:** {ene_dict['C2_Cond']+ene_dict['C2_Reb']:.2f} kW")

    st.divider()

    # 第二層：詳細數據與參考方案
    tabs = st.tabs(["💡 推薦操作模式 (代表性案例)", "📊 製程組分詳細分析"])
    ...
    with tabs[1]:
        st.markdown("#### **反應器出口組分詳細流率**")
        reactor_df = pd.DataFrame({
            "組分 (Component)": ["AA (醋酸)", "PGME (丙二醇甲醚)", "PMA (主要產品)", "Water (副產物)"],
    ...
    st.error(f"系統運行錯誤：{e}")
    st.info("請檢查模型路徑或輸入參數格式是否正確。")

# 頁尾
st.divider()
st.caption("PMA AI Optimization System (c) 2026 | 國立科技大學 碩士論文研究成果 | 模型架構：[256, 128, 64] | 數據來源：DWSIM Process Simulation")
